...
└── finetune/               # Stage 2~4: 실전
    ├── export_corpus.py
    ├── gen_queries.py
    ├── split_pairs.py       # train/eval 분할 (chunk_id 기준 + 페이지 층화)
    ├── prep_eval_human.py   # 사람 작성 평가셋 뼈대 생성·검증
    ├── evaluate.py
    ├── train_embedder.py
    ├── reindex.py
...
| 파일 | 하는 일 | 입력 → 출력 | 주의점 |
|---|---|---|---|
| `export_corpus.py` | sqlite 읽기 전용으로 청크 추출 + **페이지별 상한 적용** | `chroma.sqlite3` → `chunks.jsonl` | 위키 서버·Chroma 클라이언트·임베딩 모델 전부 불필요. `--max-per-page 60` 으로 쏠림 완화 (8절 4항) |
| `gen_queries.py` | LLM으로 질문 합성 | 청크 → `pairs.jsonl` (~2,200쌍) | `--limit` / `--resume` **필수** |
| `split_pairs.py` | train/eval 나누기 | `pairs.jsonl` → `pairs_train` / `pairs_eval` | **`chunk_id` 기준** 분할 (누수 방지) + 페이지 층화 |
| `prep_eval_human.py` | 사람 평가셋 뼈대 생성·형식 검증 | `chunks.jsonl` → `eval_human_template.jsonl` | 질문은 **사용자가 직접** 채운다 (8절 1항) |
...
- 시드 고정, 스텝별 loss 로깅.
- ~~소요 시간은 추정치일 뿐~~ → **2026-09-10 실측 완료** (`07_contrastive.py` 6절):
  **스텝당 1.10초, 에폭당 약 4.6분** (2,000쌍 기준). 추정(15~30분)보다 3~6배 빠르다.
- **★ 메모리 실측: 피크 1,628 MB.** 당초 계산(930 MB)이 파이썬+torch 베이스라인
  773 MB를 누락했었다. **학습에는 여유 RAM 최소 1.7 GB가 필요하다.**
  이 서버 여유는 1.4~2.4 GB로 변동하므로, 학습 전 `free -m` 확인과
  **Streamlit 앱 종료가 필수**다. 1.4 GB 상황에서는 OOM으로 죽는다.
...
2. **데이터가 작다.** 18개 위키 페이지 / 1,136 청크에서 나온 2천 쌍은 임베딩 파인튜닝으로는 소규모다.
   개선 폭이 미미하거나 **오히려 나빠질 수 있다.**
   다만 이 규모가 PyTorch를 배우기에는 오히려 적당하다 — 실험 한 사이클이 30분 안에 끝난다.
3. **★ 코퍼스가 한 페이지에 심하게 쏠려 있다 (2026-09-10 실측).**
   `IP 주소 대장`(page_id 33) 한 페이지가 **867청크 = 전체의 76.3%** 를 차지한다.
   나머지 17개 페이지가 269청크를 나눠 갖는다.

   | page_id | 청크 | 비중 | 제목 |
   |---|---|---|---|
   | 33 | **867** | **76.3%** | IP 주소 대장 |
   | 23 | 67 | 5.9% | 사이버분야 위기대응 실무매뉴얼 |
   | 31 | 26 | 2.3% | 정보시스템 관리대장 |
   | (나머지 15개) | 176 | 15.5% | |

   관리대장류 마크다운 표는 문장형 임베딩 모델이 원래 약한 영역이고 BM25가 더 잘 잡는다.
   그런 문서가 코퍼스의 대부분이라는 뜻이므로, 파인튜닝 효과를 낙관하기 어렵다.

4. **대응 — 상한 + 손실 마스킹 (2026-09-10 측정 후 방침 변경).**

   처음엔 "같은 페이지 청크는 내용이 거의 같아 false negative가 된다"고 판단해
   배치 구성을 제약하려 했다. **측정해보니 그 전제가 틀렸다.**
   ...
   page 33이 특별히 자기유사한 게 아니고 오히려 다른 페이지가 더 높다.
   IP 대장의 행들은 IP 주소가 전부 달라 잘 구분된다.

   또 배치 구성을 제약하면 **페이지가 18개뿐이라 batch 8 기준 33배치가 상한**이다
   (cap을 무제한으로 풀어도 동일 — 데이터가 아니라 페이지 수에 걸린다).

   **그래서 두 가지로 나눠 대응한다:**

   | 문제 | 대응 | 근거 |
   |---|---|---|
   | **학습 데이터 편향** (질문 예산의 76%를 IP 대장에) | `export_corpus.py --max-per-page 60` | cap 60이면 최대 비중 18.9%, 318청크 |
   | **false negative** (실재하지만 경미) | **손실 함수에서 같은 페이지 마스킹** | 배치 구성 제약 없이 데이터 100% 활용 |
   | **평가 왜곡** | 페이지별 지표 + **macro 평균** | 변경 없음 |

   마스킹 구현 (3줄):
   ```python
   same = page_ids[:, None] == page_ids[None, :]   # 같은 페이지면 True
   same.fill_diagonal_(False)                       # 대각선(정답)은 살린다
   sim = sim.masked_fill(same, -1e4)                # 같은 페이지는 negative에서 제외
   ```
   최악의 경우(배치 8 중 5개가 유사도 0.99인 같은 페이지) 손실 0.8899 → 0.0000 으로 검증.

   ⚠️ 배치 전체가 같은 페이지면 negative가 0개가 되어 학습 신호가 사라진다.
   cap 60이면 그 확률은 0.189⁸ ≈ 4e-6 으로 무시할 수 있다.

   **cap 60 기준 규모**: 318청크 × 질문 2개 = **636쌍** → batch 8로 **79스텝/에폭**
   → 실측 1.10초/스텝 기준 **에폭당 약 1.5분**.
5. **RAM이 진짜 빡빡하다.** 여유 1.4 GB에 스왑이 이미 1.1 GB 사용 중이다.
